...
export default function Card(props) {
  return (
    <div className=' ease-in duration-200 bg-input/[0.3] p-12 hover:bg-selected-text/[0.13] hover:shadow-lg shadow-md shadow-cyan-500/40 hover:shadow-cyan-500/60 rounded-lg'>
      ...
      <p className=' text-slate-100 md:h-36 mx-auto md:text-center md:w-72'>{props.description}</p>
    <div className='flex justify-center gap-8 text-center py-8 mt-4'>
    <button className='bg-theme ease-in duration-200 p-2 md:p-3 font-semibold hover:scale-105 hover:bg-theme/[.7]' onClick={(e) => {e.preventDefault(); window.open(props.source)}}>Source Code </button> 
    <button className='bg-theme ease-in duration-200 p-2 md:p-3 font-semibold hover:scale-105 hover:bg-theme/[.7]' onClick={(e) => {e.preventDefault(); window.open(props.preview)}}>Live Preview</button>
  </div>
